...
| **Welcome Animation**         | Personalised welcome screen on login and signup — prefetches dashboard data in background so the portal is populated when the animation ends                                                                                                                                                        |
...
| **AI Keyword Generation**     | LLM-generated search query crafted from user skills, target roles, title, and experience when the search field is empty; the generated query is returned in the response for user refinement                                                                                                        |
| **Job Search & Aggregation**  | Multi-source parallel job aggregation across major boards, full-text search, salary and recency filters, MongoDB TTL cache; result count enforced by tier                                                                                                                                           |
| **AI Cover Letters**          | Multi-provider LLM generation with automatic failover and Resilience4j circuit breakers, custom and system templates, inline editing, per-user provider preference; daily generation count enforced by tier                                                                                         |
| **Career Path Analysis**      | AI-powered career progression analysis (Gold+ only): suggests realistic next roles based on resume, skills, experience, and projects; provides per-role checkpoints, mandatory skills, and estimated timelines                                                                                      |
| **Mock Interview Prep**       | AI-driven interview session generation (Gold+ only): generates role-specific questions across behavioural, technical, situational, and culture-fit categories; evaluates answers with per-question scoring and overall session feedback                                                              |
| **Resume Optimiser**          | AI-powered resume enhancement (Gold+ only): rewrites bullet points, improves ATS keyword density, and surfaces gap analysis against a target job description                                                                                                                                        |
...
| **Payments & Refunds**        | Razorpay integration with HMAC-SHA256 signature verification; smart 7-day refund policy with feature usage accounting; dev-mode mock orders when keys are blank                                                                                                                                    |
...
| **UI Design System**          | Apple-quality design language: `rounded-xl`/`rounded-2xl` corners, `font-semibold` typography, `text-[#1d1d1f]`/`text-[#86868b]` system palette, `backdrop-blur` frosted-glass nav, soft multi-layer shadows; full dark mode parity; icon-first buttons (Lucide); consistent across all pages, modals, forms, and UI primitives |
...
| **AI Resilience**       | Resilience4j circuit breaker + retry per provider, task-aware routing   | Prevents cascading failures; free providers are tried before premium; routing adapts per task type for optimal quality and cost                                   |
...
│       │       │                        # AiProviderFactory (resilient cascade)
│       │       │                        # AiTextGenerator (functional interface)
│       │       ├── scraper/             # ScraperOrchestrator + API clients
...
│       │                                # CareerPathService, InterviewPrepService,
│       │                                # ResumeParserService, FeatureUsageService,
│       │                                # SubscriptionService, SalaryNormalizationService
│       └── test/
│           ├── java/com/kaddy/autoapply/
│           │   ├── AutoApplyApplicationTest.java
│           │   ├── config/security/
│           │   ├── controller/
│           │   ├── exception/
│           │   ├── security/
...
│       │   │                            # /cover-letters · /profile · /settings
│       │   │                            # /smart-resume · /interview-prep
│       │   └── onboarding/
│       ├── components/
│       │   ├── ui/
│       │   ├── layout/
│       │   ├── subscription/            # RefundModal
│       │   └── …/
│       ├── hooks/
│       ├── lib/
│       │   ├── api.ts
...
| Docker Desktop | Latest          | Required for Redis                                   |
...
export GROQ_API_KEY=your_groq_key           # groq.com — free tier available
export CEREBRAS_API_KEY=your_cerebras_key   # cerebras.ai — free tier available
```
...
| `AiProviderFactoryTest`      | 11    | Provider cascade, preferred provider, unavailable provider handling                  |
...
| Method | Endpoint                   | Auth     | Description                                                          |
| ------ | -------------------------- | -------- | -------------------------------------------------------------------- |
| `GET`  | `/api/jobs/search`         | Optional | Search jobs; omit `query` to trigger AI keyword generation           |
| `GET`  | `/api/jobs/{id}`           | Optional | Get job details                                                      |
| `GET`  | `/api/jobs/{id}/summarize` | Bearer   | AI-generated job summary (Gold+)                                     |
...
| ------------ | -------- | ------- | ---------------------------------------------- |
| `query`      | `string` | `""`    | Search terms; if empty, AI generates from profile |
...
| ------ | -------------------------- | ------ | ----- | -------------------------------------------------------- |
| `GET`  | `/api/career-path/analyze` | Bearer | Gold+ | AI career path analysis with role suggestions and checkpoints |

### Interview Prep

| Method | Endpoint                                   | Auth   | Tier  | Description                          |
| ------ | ------------------------------------------ | ------ | ----- | ------------------------------------ |
| `POST` | `/api/interview-prep/sessions`             | Bearer | Gold+ | Start a new AI mock interview session |
| `POST` | `/api/interview-prep/sessions/{id}/submit` | Bearer | Gold+ | Submit answers for evaluation        |
| `GET`  | `/api/interview-prep/sessions`             | Bearer | Gold+ | List past sessions (paginated)       |
| `GET`  | `/api/interview-prep/sessions/{id}`        | Bearer | Gold+ | Get a specific session with results  |
...
| Method | Endpoint                             | Auth   | Description                             |
| ------ | ------------------------------------ | ------ | --------------------------------------- |
| `GET`  | `/api/subscriptions/refund-eligibility` | Bearer | Check smart refund eligibility        |
| `POST` | `/api/subscriptions/request-refund`  | Bearer | Request refund within 7-day window      |

---

## AI Provider Configuration

The platform routes AI requests through a resilient multi-provider system with automatic failover. Provider selection is task-aware — different tasks use different provider priorities to optimise for quality, latency, and cost. Resilience4j circuit breakers prevent cascading failures; if a provider is unavailable, the next is tried automatically.

No manual provider selection is required. The system adapts in real time based on provider availability.
...
At least one provider must be configured for AI features to function. Free-tier providers are sufficient for all platform capabilities.
...
| **Authentication**      | Stateless JWT; refresh tokens stored server-side in Redis with TTL                                                                       |
...
> **MongoDB** is not a Render service — it is provided by **MongoDB Atlas**. Set `MONGODB_URI` to your Atlas connection string in the Render dashboard.
...
- **Maven dependency cache:** `--mount=type=cache,target=/root/.m2` persists the local Maven repository between builds. Only invalidated when `pom.xml` changes.
- **`.dockerignore`:** Excludes `target/`, `.git`, `.idea`, and test reports from the build context.

> **Why Ubuntu Jammy (not Alpine)?** ZGC (`-XX:+UseZGC`) requires glibc. Alpine Linux uses musl libc, which is incompatible with ZGC and causes the JVM to crash on startup.
...
| `-XX:MaxMetaspaceSize=128m`     | 128 MB          | Holds class metadata + Spring proxy classes                                               |
...
- **Eager initialisation** — lazy-init is explicitly disabled to ensure the HTTP port is bound before Render's first health-check scan.
- **MongoDB health indicator disabled** — prevents `MongoHealthIndicator` from querying the Atlas internal `local` database, which returns `Unauthorized` and causes health checks to fail.
- **AutoSearchScheduler 2-minute initial delay** — prevents the background scheduler from competing for MongoDB connections during the startup window.
- **Redis connect-timeout raised to 2000ms** — accommodates network latency between Render services during pool initialisation.
